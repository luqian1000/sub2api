...
func TestSettingService_GetPublicSettings_ExposesModelSquareFeatureSwitches(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
			SettingKeyModelSquareHomeEnabled: "true",
			SettingKeyModelSquareNavEnabled:  "true",
		},
	}
	svc := NewSettingService(repo, &config.Config{})
	...
	settings, err := svc.GetPublicSettings(context.Background())
	require.NoError(t, err)
	require.True(t, settings.ModelSquareHomeEnabled)
	require.True(t, settings.ModelSquareNavEnabled)
}
